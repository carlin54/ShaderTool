#ifndef RTPREVIEWWINDOW_H
#define RTPREVIEWWINDOW_H
...
// Embedded Vulkan preview using a custom VkDevice (RT + buffer device address). Not QVulkanWindow.
class RtPreviewWindow : public QWindow
{
    Q_OBJECT
public:
    explicit RtPreviewWindow(QVulkanInstance *inst, QWindow *parent = nullptr);
    ~RtPreviewWindow() override;
    ...
    void setRayTracePayload(const QVector<RayTraceStageBinary> &stages,
...
private:
    ...
    RtPreviewEngine m_engine;

    QMutex m_payloadMutex;
    QVector<RayTraceStageBinary> m_pendingStages;
    quint32 m_pendingMaxRecursion = 0;
