...
RtPreviewWindow::RtPreviewWindow(QVulkanInstance *inst, QWindow *parent)
    : QWindow(parent)
    , m_qInst(inst)
{
    ...
    connect(m_timer, &QTimer::timeout, this, &RtPreviewWindow::onFrameTick);
}

RtPreviewWindow::~RtPreviewWindow()
{
    m_timer->stop();
    releaseGpu();
}

bool RtPreviewWindow::event(QEvent *e)
{
    switch (e->type()) {
...
void RtPreviewWindow::updateHostUniformsForFrame()
{
...
void RtPreviewWindow::loadInstanceProcs()
{
...
void RtPreviewWindow::loadDeviceProcs()
{
...
VkFormat RtPreviewWindow::pickStorageCompatibleSurfaceFormat(VkColorSpaceKHR *outColorSpace) const
{
...
void RtPreviewWindow::releaseGpu()
{
...
void RtPreviewWindow::ensureGpu()
{
    if (m_device.isValid())
        return;
    ...
    if (df->vkCreateCommandPool(dev, &cpci, nullptr, &m_cmdPool) != VK_SUCCESS) {
        qWarning() << "RtPreviewWindow: vkCreateCommandPool failed.";
        return;
    }
    ...
    if (df->vkAllocateCommandBuffers(dev, &cbai, &m_cmdBuf) != VK_SUCCESS) {
        qWarning() << "RtPreviewWindow: vkAllocateCommandBuffers failed.";
        return;
    }
...
void RtPreviewWindow::recreateSwapchain()
{
    ...
    if (m_pfnCreateSwapchainKHR(dev, &sci, nullptr, &m_swapchain) != VK_SUCCESS) {
        qWarning() << "RtPreviewWindow: vkCreateSwapchainKHR failed.";
        return;
    }
...
void RtPreviewWindow::resetGpuErrorAnnouncement()
{
    m_gpuErrorEmitted = false;
}

void RtPreviewWindow::setPreferredGpuName(const QString &name)
{
...
void RtPreviewWindow::setRayTracePayload(const QVector<RayTraceStageBinary> &stages,
                                         quint32 maxPipelineRayRecursionDepth)
{
    QMutexLocker lock(&m_payloadMutex);
    m_pendingStages = stages;
    m_pendingMaxRecursion = maxPipelineRayRecursionDepth;
    m_payloadDirty = true;
    if (!m_timer->isActive())
        m_timer->start(16);
    requestUpdate();
...
void RtPreviewWindow::onFrameTick()
{
    renderFrame();
}

void RtPreviewWindow::exposeEvent(QExposeEvent *e)
{
...
void RtPreviewWindow::resizeEvent(QResizeEvent *e)
{
...
void RtPreviewWindow::renderFrame()
{
    ...
    QVector<RayTraceStageBinary> stagesCopy;
    quint32 maxRec = 0;
    bool dirty = false;
    {
        QMutexLocker lock(&m_payloadMutex);
        stagesCopy = m_pendingStages;
        maxRec = m_pendingMaxRecursion;
        dirty = m_payloadDirty;
    }

    QString err;
    if (dirty && !stagesCopy.isEmpty()) {
        const QSize sz(int(m_swapExtent.width), int(m_swapExtent.height));
        if (!m_engine.rebuild(&m_device, stagesCopy, sz, m_swapFormat, &err, maxRec)) {
            if (!err.isEmpty())
                qWarning() << "RtPreviewEngine:" << err;
        }
        QMutexLocker lock(&m_payloadMutex);
    ...
    df->vkWaitForFences(dev, 1, &m_inFlight, VK_TRUE, UINT64_MAX);
